    ...
    <div style={{ minHeight: '100vh', background: '#070707', display: 'flex', fontFamily: "'Satoshi', 'DM Sans', sans-serif" }}>
      <style dangerouslySetInnerHTML={{__html:`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .btn{width:100%;background:#F5A623;color:#080808;border:none;padding:15px;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;font-family:'Syne','DM Sans',sans-serif;transition:all 0.2s;letter-spacing:0.02em}
      ...
      {/* Left panel */}
      <div className="hide-m" style={{ flex: 1, background: 'rgba(245,166,35,0.04)', borderRight: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '20%', left: '-10%', width: 400, height: 400, background: 'rgba(245,166,35,0.08)', borderRadius: '50%', filter: 'blur(100px)', animation: 'float 8s ease-in-out infinite' }} />
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 26, fontWeight: 800, marginBottom: 48 }}>VYRA<span style={{ color: '#F5A623' }}>.</span></div>
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: 38, fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1.1, marginBottom: 20, color: '#fff' }}>
            Your viral content<br />intelligence hub.
          </h2>
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 16, lineHeight: 1.7, maxWidth: 380, marginBottom: 48 }}>
            Millions of viral posts. AI breakdowns. Script generation. Everything you need to create content that actually performs.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {['1M+ like posts discovered daily','AI breakdown of every viral hook','30-second script generation','TikTok, Instagram, YouTube, Reddit'].map((f, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 14, color: 'rgba(255,255,255,0.6)' }}>
      ...
      <div style={{ width: '100%', maxWidth: 480, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px 48px', position: 'relative', zIndex: 1 }}>
        <div style={{ marginBottom: 40 }}>
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 22, fontWeight: 800, marginBottom: 32, display: 'none' }}>VYRA<span style={{ color: '#F5A623' }}>.</span></div>
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: 32, fontWeight: 800, color: '#fff', marginBottom: 8, letterSpacing: '-0.02em' }}>Welcome back</h1>
          <p style={{ color: 'rgba(255,255,255,0.38)', fontSize: 15 }}>Sign in to your VYRA account</p>
        </div>
        ...
        <p style={{ textAlign: 'center', marginTop: 28, fontSize: 14, color: 'rgba(255,255,255,0.3)' }}>
          ...
          <span style={{ color: '#F5A623', cursor: 'pointer', fontWeight: 700 }} onClick={() => router.push('/signup')}>Sign up free</span>
        </p>
